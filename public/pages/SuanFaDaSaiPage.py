...
class SuanFaDaSaiPage(basepage.Page):
    ...
    def clike_xingrenchongshibie(self):
        """点击行人重识别"""
        logger.info("点击行人重识别中------------->")
        self.dr.click(self.index["suanfadasai"]["zhong_xingren"])
        try:
    ...
    def clike_arfuzhu(self):
        """点击AR辅助"""
        logger.info("点击行人重识别中------------->")
        self.dr.click(self.index["suanfadasai"]["zhong_arfuzhu"])
        try:
            code = self.dr.element_wait(self.index["suanfadasai"]["saiti_1_shujuji"])
            return bool(code)
        except Exception as e:
